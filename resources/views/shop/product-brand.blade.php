    ...
    <div class="bg-gray-50">
        ...
        <section class="bg-gradient-to-r from-blue-50 to-indigo-100 text-gray-800 py-10">
            <div class="container mx-auto px-4 text-center">
                ...
                {{-- Optionally display brand description --}}
                {{-- @if($brand->description)
                <p class="text-md text-gray-600 mt-2 max-w-2xl mx-auto">{{ $brand->description }}</p>
                @endif --}}
            </div>
        </section>
                ...
                <div class="bg-white p-6 rounded-lg shadow-sm">
                     <form action="{{ request()->url() }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-end">
                        ...
                        <div>
                            ...
                            <select id="category" name="category" class="block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 transition">
                ...
                <div id="product-container" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                     @forelse ($products as $product)
                        <div class="product-item group relative bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300">
                            ...
                            <div class="relative w-full pt-[100%]">
                                <a href="{{ route('shop.product.detail', ['slug' => $product->slug]) }}" class="absolute top-0 left-0 w-full h-full">
                                    <img 
                            ...
                            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-10 transition-all duration-300">
                                <div class="absolute bottom-0 left-0 right-0 p-4 bg-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex justify-between items-center">
                                    <p class="text-base font-semibold text-gray-900">
                                        {{ number_format($product->price_buy, 0, ',', '.') }}₫
                                    </p>
                    ...
                    @empty
                        <div class="col-span-full text-center py-12">
                            <p class="text-gray-500 text-lg">Không tìm thấy sản phẩm nào thuộc thương hiệu này.</p>
                        </div>
                    @endforelse
                </div>
