    ...
    <div>
        <form action="{{ route('admin.banner.update', ['id' => $banner->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="flex justify-between items-start mb-3 p-3 gap-5 rounded-md border">
                <h3 class="text-xl font-bold text-gray-900">Chỉnh sửa Banner</h3>
                ...
                <div class="col-span-2 bg-white p-4 rounded-md shadow">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea 
                            ...
                            name="description"
                            rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base">{{ old('description', $banner->description) }}</textarea>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        @if($banner->image)
                            <div class="mt-2">
                                <img src="{{ asset('images/banner/'.$banner->image) }}" alt="Current Image" class="w-32 h-32 object-cover">
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <select id="sort_order"
                            ...
                            @foreach($banners as $item)
                                <option value="{{ $item->id }}" {{ $banner->sort_order == $item->id ? 'selected' : '' }}>
                                    Sau: {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
