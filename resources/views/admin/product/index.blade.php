    ...
    <div class="bg-white rounded-lg shadow-md">
        ...
        <div class="p-4 border-b border-gray-200">
            <div class="flex justify-between items-center">
                ...
                <div class="space-x-2">
                    <a href="{{ route('admin.product.create') }}" class="px-3 py-2 bg-green-500 text-white rounded hover:bg-green-600 inline-flex items-center">
                        <i class="fa-solid fa-plus mr-2"></i>
                        Thêm
                    </a>
                    <a href="{{ route('admin.product.trash') }}" class="px-3 py-2 bg-red-500 text-white rounded hover:bg-red-600 inline-flex items-center">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">{{ $product->brand->name??"None"}}</td>
         
                        <td class="px-4 py-3 text-center space-x-2">
                            {{-- Status button --}}
                            @if ($product->status==1)
                                <a href="{{ route('admin.product.status', ['product'=>$product->id]) }}" class="inline-flex p-1 bg-green-600 py-1 px-1.5 text-white mx-0.5">
                                    <i class="fa-solid fa-toggle-on"></i>
                                </a>
                            @else
                                <a href="{{ route('admin.product.status', ['product'=>$product->id]) }}" class="inline-flex p-1 bg-red-600 py-1 px-1.5 text-white mx-0.5">
