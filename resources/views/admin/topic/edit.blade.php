<x-layout-admin>
    ...
    <div>
        <form action="{{ route('admin.topic.update', ['slug' => $topic->slug]) }}" method="post">
            @csrf
            @method('PUT')
            <div class="flex justify-between items-start mb-3 p-3 gap-5 rounded-md border">
                <h3 class="text-xl font-bold text-gray-900">Chỉnh sửa Chủ đề</h3>
                ...
                <div class="col-span-2 bg-white p-4 rounded-md shadow">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea 
                            ...
                            name="description"
                            rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base">{{ old('description', $topic->description) }}</textarea>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <select id="sort_order"
                            ...
                            @foreach($topics as $item)
                                <option value="{{ $item->id }}" {{ $topic->sort_order == $item->id ? 'selected' : '' }}>
                                    Sau: {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
